...
export default function ClientUsersPage() {
  ...
  const resetForm = () =>
    setForm({ idperfil: "", nombre: "", apellido: "", email: "", password: "" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        await authFetch(`/usuarios/${editingId}`, { method: "PUT", body: JSON.stringify(payload) });
        setOk("Usuario actualizado.");
      } else {
        payload.password = form.password;
        await authFetch(`/usuarios`, { method: "POST", body: JSON.stringify(payload) });
  ...
  return (
    <div>
      ...
      <h1 className="mb-4 text-2xl font-bold">👥 Usuarios (Cliente)</h1>
          <p className="text-muted-foreground text-sm max-w-2xl">
            Crea y edita diferentes usuarios y asigna perfiles de acceso dentro del sistema.
          </p>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[1000px] border">
            ...
            <tbody>
              ...
              {usuarios.length === 0 && (
                <tr><td className="p-3 text-center text-gray-500" colSpan={7}>No hay usuarios.</td></tr>
              )}
            </tbody>
          </table>
        </div>
