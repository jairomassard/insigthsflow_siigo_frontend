  ...
  useEffect(() => {
    ...
    const handleBarClick = async (tipo: "ingresos" | "egresos", data: any) => {
        ...
        try {
            ...
            const result = await authFetch(`/reportes/financiero/compras-gastos/detalle?${qs.toString()}`);
            setDetalleFacturas(result || []);
            }
        } catch (err) {
            ...
            <table className="w-full border-collapse border text-sm">
                ...
                <tr className="bg-gray-100">
                    ...
                    <>
                        ...
                        <th className="border p-2">Centro de Costo</th> {/* 👈 nuevo */}
                ...
                {detalleFacturas.length === 0 ? (
                    ...
                    if (modalTipo === "ingresos") {
                        ...
                        const isNoPagada =
                        (f.estado || "").toLowerCase() === "no pagada";

                        return (
                        <tr
                            key={i}
                            className={`hover:bg-gray-50 ${
                            isNoPagada ? "text-red-600 font-bold" : ""
                            }`}
                        >
                            <td className="border p-2">{proveedor}</td>
                            <td className="border p-2">{f.factura}</td>
                            <td className="border p-2">
                            {format(new Date(f.fecha), "dd-MM-yyyy")}
                            </td>
                            <td className="border p-2">
