...
export default function SiigoIntegrationPage() {
  ...
  const [debugCotLoading, setDebugCotLoading] = useState(false);
  const [debugCotMsg, setDebugCotMsg] = useState("");
  const [debugCotJson, setDebugCotJson] = useState<any>(null);

  const [debugCotPage, setDebugCotPage] = useState("1");
  const [debugCotPageSize, setDebugCotPageSize] = useState("10");
  const [debugCotId, setDebugCotId] = useState("");
  const [debugCotName, setDebugCotName] = useState("C-1-481");
  ...
  const consultarCotizaciones = async () => {
    setDebugCotLoading(true);
    setDebugCotMsg("");
    setDebugCotJson(null);

    try {
      const query = new URLSearchParams();
      query.set("page", debugCotPage.trim() || "1");
      query.set("page_size", debugCotPageSize.trim() || "10");

      const res = await fetchWithIdCliente(`/siigo/debug-cotizaciones?${query.toString()}`, {
        method: "GET",
      });
      ...
      setDebugCotMsg("Cotizaciones consultadas correctamente. No se guardó información.");
      setDebugCotJson(data);
    } catch (error: any) {
      setDebugCotMsg(`Error consultando cotizaciones: ${error.message}`);
    } finally {
      setDebugCotLoading(false);
    }
  };

  const consultarCotizacionPorId = async () => {
    if (!debugCotId.trim()) {
      setDebugCotMsg("Primero escribe el ID Siigo de la cotización.");
      return;
    }
    ...
    try {
      const res = await fetchWithIdCliente(
        `/siigo/debug-cotizaciones/${encodeURIComponent(debugCotId.trim())}`,
        { method: "GET" }
      );
      ...
      setDebugCotMsg("Detalle de cotización consultado correctamente.");
      setDebugCotJson(data);
    } catch (error: any) {
      setDebugCotMsg(`Error consultando detalle de cotización: ${error.message}`);
    } finally {
      setDebugCotLoading(false);
    }
  };

  const buscarCotizacionPorName = async () => {
    if (!debugCotName.trim()) {
      setDebugCotMsg("Primero escribe el número visible de la cotización, por ejemplo C-1-481.");
      return;
    }
    ...
    try {
      const query = new URLSearchParams();
      query.set("name", debugCotName.trim());
      query.set("page_size", "50");
      query.set("max_pages", "30");

      const res = await fetchWithIdCliente(
        `/siigo/debug-cotizaciones-buscar?${query.toString()}`,
        { method: "GET" }
      );
      ...
      setDebugCotMsg(
        data?.encontrada
          ? `Cotización ${debugCotName} encontrada. Copia el ID Siigo para consultar el detalle.`
          : `No se encontró la cotización ${debugCotName} en las páginas consultadas.`
      );
      setDebugCotJson(data);
    } catch (error: any) {
      setDebugCotMsg(`Error buscando cotización: ${error.message}`);
    } finally {
      setDebugCotLoading(false);
    }
  };
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 pb-12">
      ...
      >
        ...
        <details className="rounded-xl border border-slate-200 bg-slate-50 p-4">
          <summary className="cursor-pointer text-sm font-semibold text-slate-900">
            Exploración Cotizaciones API
          </summary>

          <div className="mt-4 space-y-4">
            <div className="rounded-xl border border-blue-100 bg-blue-50 p-3 text-sm leading-6 text-blue-900">
              Esta herramienta consulta cotizaciones directamente desde Siigo API en modo solo lectura.
              Sirve para validar si el JSON de Siigo permite identificar si una cotización fue convertida
              en factura electrónica. No guarda ni modifica información en InsightFlow.
            </div>

            <div className="grid gap-3 md:grid-cols-[120px_160px_auto]">
              <div>
                ...
                <input
                  value={debugCotPage}
                  onChange={(e) => setDebugCotPage(e.target.value)}
                  className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-blue-500"
                />
              </div>
              ...
              <div>
                ...
                <input
                  value={debugCotPageSize}
                  onChange={(e) => setDebugCotPageSize(e.target.value)}
                  className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-blue-500"
                />
              </div>
              ...
              <div className="flex items-end">
                <button
                  type="button"
                  onClick={consultarCotizaciones}
                  disabled={debugCotLoading}
                  className="w-full rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800 disabled:opacity-50 md:w-auto"
                >
                  {debugCotLoading ? "Consultando…" : "Consultar cotizaciones"}
                </button>
              </div>
            </div>
            ...
            <div className="grid gap-3 md:grid-cols-[1fr_auto]">
              <div>
                <label className="mb-1 block text-xs font-medium text-slate-700">
                  Buscar por número visible
                </label>
                <input
                  value={debugCotName}
                  onChange={(e) => setDebugCotName(e.target.value)}
                  placeholder="Ej: C-1-481"
                  className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-blue-500"
                />
                <p className="mt-1 text-xs text-slate-500">
                  Útil para encontrar el ID interno de Siigo de una cotización.
                </p>
              </div>
              ...
              <div className="flex items-end">
                <button
                  type="button"
                  onClick={buscarCotizacionPorName}
                  disabled={debugCotLoading}
                  className="w-full rounded-xl bg-blue-700 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-800 disabled:opacity-50 md:w-auto"
                >
                  {debugCotLoading ? "Buscando…" : "Buscar por name"}
                </button>
              </div>
            </div>
            ...
            <div className="grid gap-3 md:grid-cols-[1fr_auto]">
              <div>
                <label className="mb-1 block text-xs font-medium text-slate-700">
                  ID Siigo de la cotización
                </label>
                <input
                  value={debugCotId}
                  onChange={(e) => setDebugCotId(e.target.value)}
                  placeholder="UUID interno de Siigo"
                  className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-blue-500"
                />
                <p className="mt-1 text-xs text-slate-500">
                  Después de buscar C-1-481 o C-1-469, copia aquí el ID para consultar el detalle.
                </p>
              </div>
              ...
              <div className="flex items-end">
                <button
                  type="button"
                  onClick={consultarCotizacionPorId}
                  disabled={debugCotLoading}
                  className="w-full rounded-xl bg-emerald-700 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-800 disabled:opacity-50 md:w-auto"
                >
                  {debugCotLoading ? "Consultando…" : "Consultar detalle"}
                </button>
              </div>
            </div>

            <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs leading-5 text-amber-900">
              <strong>Prueba recomendada:</strong> primero busca <strong>C-1-481</strong>, consulta su
              detalle por ID y revisa si aparece alguna referencia a <strong>FV-2-1924</strong>. Luego
              repite el proceso con <strong>C-1-469</strong>. Si el primer JSON trae relación con factura
              y el segundo no, podemos construir el módulo de conversión con confianza.
            </div>

            {debugCotMsg && <MessageBox message={debugCotMsg} type="info" />}
            {debugCotJson && <JsonBlock title="Respuesta JSON cotizaciones Siigo" data={debugCotJson} />}
          </div>
        </details>
